...
{
    public class PlayerHighHurricaneKick : MonoBehaviour
    {
        [SerializeField]
        GameObject Player;
        ...
        {
            // Animation
            #region EnterHighHurricaneKick
            ObservableStateMachineTrigger
                .OnStateEnterAsObservable()
                .Where(x => x.StateInfo.IsName("Base Layer.HighHurricaneKick"))
                .Do(x => Debug.Log("Enter HurricaneKick"))
                .Subscribe(_ => coroutineStore = StartCoroutine(HighHurricaneKickCoroutine()));
            #endregion
            #region HighHurricaneKick->Stand
            ObservableStateMachineTrigger
                ...
                .Subscribe(_ =>
                {
                    Animator.SetBool("IsHighHurricaneKick", false);
                    Animator.SetBool("IsStanding", true);
                    hasFinished = false;
                    PlayerState.hasInputedHighHurricaneKickCommand.Value = false;
                });
            #endregion
            #region HighHurricaneKick->SupineJumpingDamage
            ObservableStateMachineTrigger
                .OnStateUpdateAsObservable()
                .Where(x => x.StateInfo.IsName("Base Layer.HighHurricaneKick"))
            ...
            #endregion
            #region HighHurricaneKick->ProneJumpingDamage
            ObservableStateMachineTrigger
                .OnStateUpdateAsObservable()
                .Where(x => x.StateInfo.IsName("Base Layer.HighHurricaneKick"))
        ...
        IEnumerator HighHurricaneKickCoroutine()
        {
            // Startup
            Animator.speed = 1f;
            ...
            for (int i = 0; i < startup; i++)
            {
                if (i < 2)
                {
                    ...
                    {
                        Rigidbody2D.velocity = new Vector2(5f, 2f);
                    }
                    else
                    {
                        Rigidbody2D.velocity = new Vector2(-5f, 2f);
